...
- [x] Reactive compaction on `ProviderError::ContextLengthExceeded` (retries capped by a per-reply counter on the op — compaction erases the error messages, so the budget can't be derived from the conversation)
