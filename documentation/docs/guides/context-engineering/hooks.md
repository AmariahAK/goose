...
| `matcher_context` | String the rule's `matcher` is tested against, such as the tool name on tool events, the prompt text on `UserPromptSubmit`, or the assistant response text on `AfterAgentResponse`. |
| `tool_name` | Name of the tool, on tool events. |
| `tool_input` | Input arguments passed to the tool, on tool events. |
| `message` | Prompt text on `UserPromptSubmit`, or assistant response text on `AfterAgentResponse`. |
